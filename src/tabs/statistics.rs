...
impl EventSM {
    ...
    fn add_time(
        entry: hash_map::Entry<'_, i32, WorkDuration>,
        start_time: NaiveDateTime,
        end_time: NaiveDateTime,
    ) {
        let current_work_time = entry.or_insert(WorkDuration::zero());

        let additional_work_time = WorkDuration::from_start_end_time(start_time, end_time);
        *current_work_time = current_work_time
            .checked_add(&additional_work_time)
            .unwrap();
    }

    fn process(
        &mut self,
        time_by_staff: &mut HashMap<i32, WorkDuration>,
        staff_member: &StaffMember,
        event: &WorkEventT,
    ) {
        match *self {
            EventSM::Away => match event.event {
                WorkEvent::StatusChange(uuid, WorkStatus::Working) => {
                    if staff_member.uuid().eq(&uuid) {
                        *self = EventSM::Working(event.created_at)
                    }
                }
                _ => {}
            },
            EventSM::Working(start_time) => match event.event {
                WorkEvent::StatusChange(uuid, WorkStatus::Away)
                    if staff_member.uuid().eq(&uuid) =>
                {
                    EventSM::add_time(
                        time_by_staff.entry(staff_member.uuid()),
                        start_time,
                        event.created_at,
                    );
                    *self = EventSM::Away;
    ...
    pub fn update(&mut self, shared: &mut SharedData, message: StatsMessage) {
        match message {
            StatsMessage::Generate => {
                ...
                let mut time_by_staff: HashMap<i32, WorkDuration> = HashMap::new();
                let events = stechuhr::load_events(&shared.connection);

                for staff_member in staff.iter() {
                    let mut eval_state = EventSM::new();

                    for event in events.iter() {
                        eval_state.process(&mut time_by_staff, staff_member, event);
                    }
                ...
                let staff_hours: Vec<PersonHours> = time_by_staff
                    .iter()
                    .map(|(k, t)| {
                        let name = &StaffMember::get_by_uuid(staff, *k).unwrap().name;
